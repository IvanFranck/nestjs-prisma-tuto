...
export class AuthService {
    ...
    async register(dto: RegisterDto){
        try {
            ...
            if(error instanceof PrismaClientKnownRequestError) {
                if (error.code === 'P2002') {
                    throw new ConflictException(`User with this email "${dto.email}" already exist`)
                }
            }
            throw new InternalServerErrorException('Error while register user. Retry later')
        }
    }
    ...
    async getUserInfo(userId: number) {
        try {
            ...
            throw new InternalServerErrorException('Error while getting user infos. retry later')
        }
    }
    ...
    async logout(userId: number) {
        try {
            ...
            if (!user) {
                throw new BadRequestException(`Le user avec l'id ${userId} n'existe pas`)
            }
            ...
            throw new InternalServerErrorException('Error while logging out user. retry later')
        }
    }
    ...
    private generateTokens(user: User): Promise<[string, string]> {
        ...
        return Promise.all([
            this.jwtService.signAsync(payload, {expiresIn: '15m', secret: this.config.getOrThrow<string>('JWT_SECRET')}),
            this.jwtService.signAsync(payload, {expiresIn: '7d', secret: this.config.getOrThrow<string>('REFRESH_JWT_SECRET')})
        ])
    }
